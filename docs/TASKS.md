...
- [x] 已按最新要求撤销默认多轮/多维度 LLM，新增 `generate_enterprise_report_single`，改为默认 standard 核心采集、按条件升级 deep，再只调用一次 LLM 生成完整 `scoring_json`。
...
- [x] Default single-stage report generation now starts with `collection_mode=standard` and only escalates to `deep` when explicit triggers are detected.
